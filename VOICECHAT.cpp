...
// 唯一的一个 CVOICECHATApp 对象

CVOICECHATApp theApp;
CString gstrGUI;
CString gstrPath; //配置文件绝对路径
// CVOICECHATApp 初始化
// 创建Dump文件  
//   
...
BOOL CVOICECHATApp::InitInstance()
{
	...
	{
		MessageBox(NULL, "初始化日志文件失败!", "提示", MB_OK);
		return FALSE;
	}
	//注册dmp服务
	...
	*p = 0;
	...
	if (183 == err)
	{
		CLogRecord::WriteRecordToFile("软件重复启动,失败!");
#ifndef DEBUG 
		return FALSE;
#endif
	}
	...
	CLogRecord::WriteRecordToFile("--------软件正常启动--------");
	dlg.RunModalLoop();
	if (nResponse == IDOK)
	{
		// TODO: 在此放置处理何时用
...
int CVOICECHATApp::ExitInstance()
{
	...
	CLogRecord::WriteRecordToFile("--------软件正常退出--------");
